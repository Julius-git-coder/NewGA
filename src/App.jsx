...
// };
// export default App;
// App.jsx - Firebase Auth Integration with Role-Based Routing


import React, { useState, useEffect } from "react";
import { Routes, Route, Navigate, useNavigate } from "react-router-dom";
import { onAuthStateChanged } from "firebase/auth";
import { auth, getUserRole } from "../Service/FirebaseConfig";
...
import Administrator from "./Components/Administrator"; // Admin Dashboard
...
const ProtectedRoute = ({ children, allowedRole, userRole, loading }) => {
  if (loading) {
  ...
  if (allowedRole && userRole !== allowedRole) {
    ...
    return <Navigate to={userRole === "admin" ? "/Administrator" : "/dashboard"} replace />;
  }
  ...
  return (
    <Routes>
      ...
      <Route 
        ...
        element={
          user ? (
            <Navigate to={userRole === "admin" ? "/Administrator" : "/dashboard"} replace />
          ) : (
            <Navigate to="/student-signup" replace />
          )
        } 
      />
      ...
      <Route
        path="/dashboard"
        element={
          <ProtectedRoute allowedRole="student" userRole={userRole} loading={loading}>
            <Dashboard />
          </ProtectedRoute>
        }
      />
      ...
      <Route
        path="/Administrator"
        element={
          <ProtectedRoute allowedRole="admin" userRole={userRole} loading={loading}>
            <Administrator />
          </ProtectedRoute>
        }
      />
